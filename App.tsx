...
import { 
  ...
  Play, Pause, RotateCcw, FileText, ChevronRight, AlertCircle, Volume2, History
} from 'lucide-react';
...
export default function App() {
  ...
  const [xp, setXp] = useState<number>(() => Number(localStorage.getItem('tempo_xp')) || 0);
  ...
  const [notes, setNotes] = useState<string>(() => localStorage.getItem('tempo_notes_draft') || localStorage.getItem('tempo_notes') || '');
  const [journalHistory, setJournalHistory] = useState<JournalPage[]>(() => {
    const saved = localStorage.getItem('tempo_journal_history');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return [];
...
if (newlyFailed.length > 0) {
  ...
  const handleAddTask = (e: React.FormEvent) => {
    ...
    setTasks(prev => [...prev, newTask].sort((a, b) => {
      const difficultyWeight = { 'صعب': 3, 'متوسط': 2, 'سهل': 1 };
  ...
  const saveJournalPage = () => {
    ...
    const nextPageNumber = journalHistory.length + 1;
  ...
  const deleteJournalPage = (id: string) => {
    setJournalHistory(prev => prev.filter(page => page.id !== id).map((page, index) => ({ ...page, pageNumber: index + 1 })));
  };
        ...
        <section className="bg-gradient-to-br from-slate-900/90 to-slate-950 border border-slate-800/80 rounded-3xl p-5 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            ...
            <div className="text-right">
              <p className="text-[11px] text-slate-400">{lang === 'ar' ? 'المتبقي من اليوم' : 'Remaining Today'}</p>
          ...
          <form onSubmit={handleAddTask} className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              ...
              <div className="space-y-1">
                <label className="block text-[11px] font-semibold text-slate-400 px-1">
                  {lang === 'ar' ? 'موعد إنهاء المهمة' : 'Task Deadline'}
                </label>
                <input 
                  type="datetime-local"
                value={newTaskDeadline}
                onChange={e => setNewTaskDeadline(e.target.value)}
                  className="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-300 focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <span>{lang === 'ar' ? 'المذكرات اليومية' : 'Daily Notes'}</span>
                </button>
              </div>
...
{/* Full Task View Tab Modal / Page */}
      ...
      {/* Notes / Daily Notes Tab Modal / Page */}
      {activeTab === 'notes' && (
        <div className="fixed inset-0 z-40 bg-[#090d16] overflow-y-auto p-4 md:p-8 animate-fade-in">
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div>
                <h2 className="text-xl font-bold text-white">{lang === 'ar' ? 'المذكرات اليومية' : 'Daily Notes'}</h2>
            ...
            <div className="bg-[#f5ebd6] text-slate-900 rounded-3xl p-6 md:p-8 shadow-2xl border-4 border-[#e2d2b5] relative overflow-hidden min-h-[500px]">
              ...
              <div className="relative z-10 flex items-center justify-between mb-4 pl-6">
                <div>
                  <p className="text-xs font-bold text-slate-500">
                    {lang === 'ar' ? `الصفحة ${journalHistory.length + 1}` : `Page ${journalHistory.length + 1}`}
                  </p>
                  <p className="text-[11px] text-slate-400">
                    {lang === 'ar' ? 'اكتب ملاحظاتك ثم احفظ الصفحة' : 'Write your notes, then save the page'}
                  </p>
                </div>
                <button
                  onClick={saveJournalPage}
                  disabled={!notes.trim()}
              ...
              <textarea 
                ...
                placeholder={lang === 'ar' ? 'اكتب ملاحظاتك وأفكارك هنا...' : 'Write your notes and thoughts here...'}
                className="relative z-10 w-full h-[390px] bg-transparent resize-none focus:outline-none text-slate-900 text-base leading-[2.5rem] font-medium pl-6"
                style={{
            ...
            <section className="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 shadow-xl">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-sm font-bold text-white">{lang === 'ar' ? 'سجل المذكرات' : 'Notes History'}</h3>
                  <p className="text-xs text-slate-400 mt-1">{lang === 'ar' ? `عدد الصفحات: ${journalHistory.length}` : `Pages: ${journalHistory.length}`}</p>
                </div>
                <History className="w-5 h-5 text-amber-400" />
              </div>
              {journalHistory.length === 0 ? (
                <p className="text-xs text-slate-500 text-center py-6">
                  {lang === 'ar' ? 'لا توجد مذكرات محفوظة بعد.' : 'No saved notes yet.'}
                </p>
              ) : (
                <div className="space-y-3">
                  {[...journalHistory].reverse().map(page => (
                    <div key={page.id} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                      <div className="flex items-center justify-between gap-3 mb-2">
                        <div className="text-xs font-bold text-amber-400">
                          {lang === 'ar' ? `صفحة ${page.pageNumber}` : `Page ${page.pageNumber}`}
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-[11px] text-slate-500">
                            {new Date(page.date).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US')}
                          </span>
                          <button onClick={() => deleteJournalPage(page.id)} className="text-slate-500 hover:text-rose-400 transition-colors">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                      </div>
                      <p className="text-sm text-slate-300 whitespace-pre-wrap leading-6">{page.content}</p>
                    </div>
                  ))}
                </div>
              )}
